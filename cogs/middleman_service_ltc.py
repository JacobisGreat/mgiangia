...
class AmountConfirmationLTCView(View):
    ...
    async def correct_button(self, interaction: discord.Interaction, button: Button):
        ...
        if self.sending_user.id in self.correct_responses and self.receiving_user.id in self.correct_responses:
            await self.delete_correct_response_messages()
            await self.call_ltc_service(interaction, self.channel, self.amount, self.sending_user, self.receiving_user, self.bot)
            await interaction.response.defer()
        else:
            await interaction.response.defer()
    ...
    @staticmethod
    async def call_ltc_service(interaction, channel, amount, sending_user, receiving_user, bot):
        ltc_service_cog = bot.get_cog('LTCService')
        if ltc_service_cog:
            await ltc_service_cog.send_final_steps(channel, amount, sending_user, receiving_user)
        else:
