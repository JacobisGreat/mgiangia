...
class InvoicePasteButtonView(View):
    def __init__(self, amount, address, currency):
    ...
    async def paste_button(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.followup.send(f"{self.amount:.6f} {self.currency}", ephemeral=False)
...
class ETHService(commands.Cog):
    ...
    async def send_payment_invoice(self, channel, amount, sending_user):
        ...
        await channel.send(content=f"{sending_user.mention}", embed=payment_invoice_embed, view=InvoicePasteButtonView(total_amount, "0x6DcfaA805d8f67cBD7E43aa31bDAE7aB51F8099b", "ETH"))
...
class AmountConfirmationETHView(View):
    ...
    async def send_final_confirmation(self, interaction):
        ...
        exchange_rate = await self.bot.get_cog('ETHService').fetch_exchange_rate("ethereum")
        ...
        payment_invoice_embed.set_footer(text=f"Exchange Rate: 1 ETH = ${exchange_rate:.2f} USD")
        payment_invoice_embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/1153826027714379866/1175266198217306112/ethereum-eth-badge-5295534-4414739.png")
        await self.channel.send(content=f"{self.sending_user.mention}", embed=payment_invoice_embed, view=InvoicePasteButtonView(total_amount, "0x6DcfaA805d8f67cBD7E43aa31bDAE7aB51F8099b", "ETH"))
    ...
    async def send_amount_request_embed(self, channel):
        ...
        try:
            ...
            try:
                ...
                await channel.send(embed=discord.Embed(description="Invalid amount format. Please enter a valid number.", color=15608876))
                await self.send_amount_request_embed(channel)
        except:
