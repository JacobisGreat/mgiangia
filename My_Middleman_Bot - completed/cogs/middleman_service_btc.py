...
class ConfirmationBTCView(View):
    ...
    async def handle_final_confirmation(self, interaction):
        ...
        btc_service = self.bot.get_cog("BTCService")
        if btc_service:
            await btc_service.send_final_steps(self.role_embed_message.channel, self.amount, self.sending_user, self.receiving_user)
...
class AmountConfirmationBTCView(View):
    ...
    async def send_final_steps(self, interaction):
        btc_service = self.bot.get_cog("BTCService")
        if btc_service:
            await btc_service.send_final_steps(self.channel, self.amount, self.sending_user, self.receiving_user)
...
async def setup(bot):
    await bot.add_cog(RoleSelectionBTCView(bot))
